...
function parseAllPairs(html) {
  ...
  const labels = [
    ...
    "Payout Ratio", "Dividend Growth", "Dividend Growth(1Y)", "Expense Ratio", "Assets", "PE Ratio",
  ...
  for (let i = 0; i < lines.length - 1; i++) {
    for (const label of labels) {
      if (lines[i] === label || lines[i].startsWith(label)) {
        // Skip parenthetical suffixes like "(1Y)" on the next line
        let nextIdx = i + 1;
        let nextVal = lines[nextIdx];
        if (nextVal && /^\([^)]+\)$/.test(nextVal) && nextIdx + 1 < lines.length) {
          nextIdx++;
          nextVal = lines[nextIdx];
        }
        const noise = ["News","Trending","Articles","Home","Stocks","ETFs","IPOs","Tools","Collapse","Log In","Sign Up","Chart","History","Watchlist","Overview","Holdings","Dividends","Full Chart","Compare","Market Movers","Open in Dividend Calculator"];
...
export async function getEtfDividend(ticker) {
  ...
  return {
    ...
    divGrowth: s["Dividend Growth(1Y)"]||s["Dividend Growth"]||"-",
  };
}
